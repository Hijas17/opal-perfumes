...
export default function SignupForm() {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-muted mb-1.5">Phone <span className="text-muted-2 text-xs">(optional)</span></label>
        <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} autoComplete="tel"
          className="w-full border border-line rounded px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold focus:border-transparent" />
      </div>
      <div>
        <label className="block text-sm font-medium text-muted mb-1.5">Password <span className="text-muted-2 text-xs">(min 6 chars)</span></label>
