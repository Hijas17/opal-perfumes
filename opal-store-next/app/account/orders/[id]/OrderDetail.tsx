    ...
    <div className="pt-16 md:pt-0 min-h-screen">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 mb-8">
          <div>
            <h1 className="font-display text-3xl sm:text-4xl font-semibold text-ink">Order {order.order_number}</h1>
            <p className="text-sm text-muted mt-1">
              Placed {order.created_at ? new Date(order.created_at).toLocaleString('en-AE', { dateStyle: 'medium', timeStyle: 'short' }) : ''}
            </p>
          </div>
          <span className={`text-xs font-medium px-3 py-1 rounded-full capitalize ${STATUS_BADGES[order.status] || 'bg-surface-2 text-muted'}`}>
            {order.status}
          </span>
        </div>
                ...
                <ol className="space-y-4">
                  {order.status_history.map((h, i) => (
                    <li key={i} className="flex gap-3 text-sm">
                      ...
                      <div>
                        <p className="font-medium text-ink capitalize">{h.status}</p>
          ...
          <div className="space-y-6">
            ...
            <Card title="Shipping To">
              <address className="not-italic text-sm text-muted leading-relaxed">
                ...
                {order.shipping.notes && <p className="mt-2 text-xs text-muted italic">Note: {order.shipping.notes}</p>}
              </address>
            </Card>
          </div>
        </div>
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-surface rounded-[var(--radius-card)] p-5 shadow-[var(--shadow-card)]">
